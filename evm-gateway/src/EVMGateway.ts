...
export class EVMGateway<T extends ProvableBlock> {
  ...
  async createProofs(address: string, paths: string[][]): Promise<string> {
    ...
    for(let i = 0; i < paths.length; i++) {
      requests.push(this.getValueFromPath(block, address, paths[i], requests.slice()));
    }
  ...
  private async computeFirstSlot(path: string[], requests: Promise<StorageElement>[]): Promise<{slot: bigint, isDynamic: boolean}> {
  ...
  private async getDynamicValue(block: T, address: string, slot: bigint): Promise<StorageElement> {
    const firstValue = ethers.getBytes(await this.proofService.getStorageAt(block, address, slot));
  ...
  private async getValueFromPath(block: T, address: string, path: string[], requests: Promise<StorageElement>[]): Promise<StorageElement> {
    ...
    if(!isDynamic) {
      return {
        slots: [slot],
        isDynamic,
        value: memoize(() => this.proofService.getStorageAt(block, address, slot))
      }
    } else {
